...
(async function() {
  for (const uri of candidates) {
    try {
      const conn = await mongoose.createConnection(uri, { dbName: uri.split('/').pop() }).asPromise();
      console.log('\n[try db] connected to', uri);
      for (const id of ids) {
        try {
          const res = await conn.model('Trip', new mongoose.Schema({}, { strict: false })).findById(id).lean();
